...
struct BnBNode {
    vector<int> path;
    int g;
    bool operator<(const BnBNode&o) const { return g > o.g; }
};

void BranchAndBound(int start,int goal){
    priority_queue<BnBNode> pq;
    pq.push({{start},0});
    while(!pq.empty()){
        ...
        for (auto &p : weightedAdj[last]){
            if (find(cur.path.begin(),cur.path.end(),p.first)==cur.path.end()){
                auto np = cur.path; np.push_back(p.first);
                pq.push({np, cur.g+p.second});
            }
        }
    }
}
